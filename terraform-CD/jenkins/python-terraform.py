...
def main():
    ...
    if args.action == "apply":
        ...
        #return apply results to the PR
        merged_pr = None
        for pr in repo.get_pulls(state="closed", head=f"{repo.owner.login}:{NEW_BRANCH}"):
            merged_pr = pr
            break
        if merged_pr:
            merged_pr.create_issue_comment(f"### Terraform Apply Results:\n```\n{apply_result}\n```")
